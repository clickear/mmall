...
public class WyMusic implements IMusic {
    ...
    private static List<MusicSong> search(String key, int page, int size) {
        try {
            ...
            MusicSongs = GetListByJson(songs);
    ...
    private static List<MusicSong> GetListByJson(List<NeteaseDatas.ResultBean.SongsBean> songs) throws Exception {
        ...
        for (NeteaseDatas.ResultBean.SongsBean song : songs) {
            ...
            if (maxbr == 999000) {
                ...
                if (!flac.contains(".mp3")) {
                    ...
                    MusicSong.setFlacUrl(flac);
                } else {
                    MusicSong.setBitRate("320K");
                }
                MusicSong.setSqUrl(GetPlayUrl(SongId, "320000"));
                MusicSong.setHqUrl(GetPlayUrl(SongId, "192000"));
                MusicSong.setLqUrl(GetPlayUrl(SongId, "128000"));
            ...
            if (songsBean.getFee() == 4 || songsBean.getPrivilege().getSt() != 0) {
                if (MusicSong.getBitRate().equals("无损")) {
                    MusicSong.setBitRate("320K");
                }
            }
